...
export abstract class MonitoringService {
    ...
    public getFilterDates(filterDto: Partial<MonitoringBaseFilterDto>): { fromDate: Date, toDate: Date } {
        const fromDate = filterDto.fromDate ? moment(filterDto.fromDate).toDate() : moment().subtract(1, 'd').toDate();
    ...
    public getRange(startDate: moment.MomentInput, endDate: moment.MomentInput): any[] {
        ...
        if (moment(startDate).isSame(endDate, 'hour')) {
            type = 'minutes';
        } else if (moment(startDate).isSame(endDate, 'day') || moment(endDate).diff(startDate, 'hour') <= 24) {
            type = 'hours';
        } else if (moment(startDate).isSame(endDate, 'month') || moment(endDate).diff(startDate, 'days') < 31) {
            type = 'days';
        } else {
            type = 'month';
        }
