...
export class SequelizeMonitoringAnalyzeService extends MonitoringService {
    ...
    public async findAllRequests(filterDto: MonitoringRequestFilterDto): Promise<MonitoringListResponse<SequelizeRequestLog>> {
    ...
    public async findAllDb(filterDto: MonitoringSequelizeFilterDto): Promise<MonitoringListResponse<SequelizeDBLog>> {
        const { fromDate, toDate } = this.getFilterDates(filterDto);
    ...
    public async findDbOptions(): Promise<any> {
        const tableNames = await this.dbLog.findAll({
    ...
    public async findAllJobs(filterDto: MonitoringJobFilterDto): Promise<MonitoringListResponse<SequelizeJobLog>> {
        const { fromDate, toDate } = this.getFilterDates(filterDto);
        const condition = { createdAt: { [Op.between]: [fromDate, toDate] } };
    ...
    public async analyzeRequests(filterDto: MonitoringBaseFilterDto): Promise<any> {
        ...
        const requests = await this.requestLog.findAll({ where: condition })
        ...
        for (let i = 0; i < durationBoundaries.length - 1; i++) {
            try {
                const list = requests.filter(e => durationBoundaries[i] <= +e['duration'] && +e['duration'] < durationBoundaries[i + 1]);
                if (list.length != 0) {
                    duration.push({
